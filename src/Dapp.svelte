<script> 
    ...
    // TODO change
    const CONTRACT_ADDRESS = '0x53829f6C7F3Bb332557C7C647Cf22ed9a4c45033';

    let provider, signer, contract, account, chainId;
    let proof = [];

    // TODO change
    let requireChainId = 42;
    
    let totalSupply, fee, currentId, numberOfTickets;
    ...
    async function updateEthers() {
        ...
        let _chainId = await provider.getNetwork();
    ...
    async function buyPublicTicket(){
        if(!contract) alert("Connect to Rinkeby on Metamask!");
        else{
            if(chainId != requireChainId);
            else{
                try{
                await contract.buyTicket(numberOfTickets, {value:(fee*numberOfTickets).toString()});
                } catch(error){
    ...
    async function buyWhitelistTicket(){
        if(!contract) alert("Connect to Rinkeby on Metamask!");
        else{
            if(chainId != requireChainId);
            else{
                ...
                const res = await contract.claimWhitelistTicket(proof);
                } catch(error){
                    console.log(error.message);
                }
            }
    ...
    async function getLotteryConfig(){
        if(!contract) alert("Connect to Rinkeby on Metamask!");
            ...
            if(chainId != requireChainId);
                ...
                [,totalSupply,fee] = await contract.lotteryConfig();
    ...
    //             params: [{ chainId: requiredChainIdHex }], // Hexadecimal version of 80001, prefixed with 0x
    //         });
    //     } catch(error){
    //         console.log(error.message);
    //     }
    ...
    function setNotificationActiveStatus(state) {
		dispatch('message', {
			status: state
		});
	}
...
<p>{currentId}/{totalSupply}</p>
<p>Fee : {fee/10**18} ETH</p>
{/if}

<!-- <button on:click={getValue}>GET VALUE</button>
<p>Value = {value}</p> -->
{/if}

{#if contract}
<div class="flexcontainer"><button id="metamask_button" on:click={buyPublicTicket} >Buy Ticket</button>
    <input type="number" style="color:black" bind:value={numberOfTickets} /></div>

    <div class="flexcontainer"><button id="metamask_button" on:click={buyWhitelistTicket} >Claim Ticket</button>
        </div>
{/if}
